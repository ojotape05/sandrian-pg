<?php 
    ...
    if(in_array($extensao, $formatosPermitidos)):
        ...
        if(empty($tipo) or empty($imagem) or empty($fornecedor) or empty($nome) or empty($descricao) or empty($link) or empty($custo) or empty($lanceInicial) or empty($estoque) or empty($ofertaSemLance)):
            ...
            *Lance Inicial: R$$lanceInicial*";
            ...
            if(mysqli_num_rows($resultado) > 0): //se houver o tipo especificado
                ...
                $sql = "INSERT INTO faca (estoquedisponivel,lanceInicial,nome,descricao,img,linkFotos,permitir_planceinicial,tipofaca,fornecedor,custo) VALUES ('$estoque','$lanceInicial','$nome','$descricao','$imagem','$link','$ofertaSemLance','$cod_tipofaca','$fornecedor','$custo')";
                $resultado = mysqli_query($connect,$sql);
