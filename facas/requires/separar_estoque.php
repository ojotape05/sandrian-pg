<?php
    ...
    if ($permitir_lanceinicial == 1){
        ...
        echo $lanceinicial;
        if ($lanceinicial == 'sim'){
            $resultado = pg_query($connect,"CALL ColocarEmLeilaoLI('$quantidade_separar','$codigo_faca','$codigo_usuario', '0')");

        } else {
            $resultado = pg_query($connect,"CALL ColocarEmLeilao('2','$codigo_faca','$codigo_usuario')");
        }
    }else{

        $resultado = pg_query($connect,"CALL ColocarEmLeilao('$quantidade_separar','$codigo_faca','$codigo_usuario')");
    }
