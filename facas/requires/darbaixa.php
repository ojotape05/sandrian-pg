...
///O erro se trata de usar duas querys na mesma conexão, pg_next_result($connect) aceita uma nova query, mas caso dê errado tente
//fechar a conexão e abrir uma nova para cada query
    include("../requires/conexao.php");
    if($_POST):
        $codigo_faca = $_POST['codigo_faca'];
        $codigo_usuario = $_POST['codigo_usuario'];
        $result = pg_query($connect,"CALL getFacasEmProcessamentoUsuario('$codigo_faca','$codigo_usuario')");
            ...
            if(isset($_POST["baixa".$codigo])):
                ...
                if($baixa == 'confirma'):
                    if(isset($_POST["comprador".$codigo]) and isset($_POST["valor_venda".$codigo])):
                        ...
                        pg_next_result($connect);
                        $resultado = pg_query($connect,"CALL ConfirmarCompra('$codigo_processamento','$comprador_processamento','$valor_venda','$obs', '$metodo_pag')");
                    endif;
                elseif($baixa == 'cancela'):
                    pg_next_result($connect);
                    $codigo_processamento = $_POST["codigo".$codigo];
                    $resultado = pg_query($connect,"CALL cancelarBaixa('$codigo_processamento','$codigo_faca')");
                endif;
            endif;
